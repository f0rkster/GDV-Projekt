...
namespace
{
    class CApplication : public IApplication
    {
        ...
        private:
            ...
            static constexpr int s_MaxNumberOfCubes = 100;
        ...
        private:
            ...
            std::array<SCube, s_MaxNumberOfCubes> m_Cubes;
            SSphere                               m_Sphere;
            SKeyState                             m_KeyState;

        private:

            SSphere::EState CalcState(SSphere::EState _CurrentState, unsigned int _Key);
...
namespace
{
    ...
    {
        m_Sphere.m_Speed          = 0.01f;
        m_Sphere.m_State          = SSphere::Roll;
        m_Sphere.m_RotationZ      = 0.0f;
        m_Sphere.m_Translation[0] = 0.0f;
        m_Sphere.m_Translation[1] = 0.0f;
        m_Sphere.m_Translation[2] = 0.0f;

        float CubePositionX = 10.0f;

        for (SCube& rCube : m_Cubes)
        {
            rCube.m_Scale[0] = 1.0f;
            rCube.m_Scale[1] = 1.0f;
            rCube.m_Scale[2] = 1.0f;

            rCube.m_Translation[0] = CubePositionX;  // Verteilung der Würfel in x-Richtung
            rCube.m_Translation[1] = 1.0f;
            rCube.m_Translation[2] = 1.0f;

            CubePositionX = CubePositionX + 10.0f;
        }
    ...
    bool CApplication::InternOnUpdate()
    {
        ...
        switch (m_Sphere.m_State)
        {
            case SSphere::Roll:
            {
                if (m_KeyState.m_IsSpaceDown)
                {
                    m_Sphere.m_State = SSphere::Up;
                }

                m_Sphere.m_Translation[0] = m_Sphere.m_Translation[0] + m_Sphere.m_Speed;
            }
            break;
            ...
            {
                m_Sphere.m_Translation[0] = m_Sphere.m_Translation[0] + m_Sphere.m_Speed;
                m_Sphere.m_Translation[1] = m_Sphere.m_Translation[1] + m_Sphere.m_Speed;

                if (m_Sphere.m_Translation[1] >= 2.0f)
                {
                    m_Sphere.m_State = SSphere::Down;
                }
            }
            break;
            ...
            case SSphere::Down:
            {
                ...
                if (m_Sphere.m_Translation[1] <= 0.0f)
                {
                    m_Sphere.m_State = SSphere::Roll;
    ...
    bool CApplication::InternOnFrame()
    {
        ...
        for (SCube& rCube : m_Cubes)
        {
            GetTranslationMatrix(rCube.m_Translation[0], 0.0f, 0.0f, WorldMatrix);
        ...
        // Draw sphere.
        // -----------------------------------------------------------------------------
        GetTranslationMatrix(m_Sphere.m_Translation[0], m_Sphere.m_Translation[1], 0.0f, WorldMatrix);
